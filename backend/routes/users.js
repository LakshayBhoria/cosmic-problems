...
// Search users. Firestore has no substring search, so this matches on a
// case-sensitive prefix of username or full_name (the standard Firestore
// "range query" trick) rather than SQL's substring LIKE. For real full-text
// search, pair this with Algolia/Typesense later.
...
router.put('/me/profile', requireAuth, async (req, res, next) => {
  try {
    ...
    await db.collection('users').doc(req.user.id).update({
      full_name: full_name ?? '', bio: bio ?? '', field_of_interest: field_of_interest ?? '',
...
router.get('/:id/followers', async (req, res, next) => {
  try {
    const snap = await db.collection('follows')
      .where('following_id', '==', req.params.id).where('status', '==', 'accepted')
...
router.get('/:id/following', async (req, res, next) => {
  try {
    const snap = await db.collection('follows')
      .where('follower_id', '==', req.params.id).where('status', '==', 'accepted')
      .orderBy('created_at', 'desc').get();
    const users = await Promise.all(snap.docs.map(async (d) => {
