...
router.post('/', requireAuth, uploadPostMedia.single('media'), async (req, res, next) => {
  try {
    ...
    // `overlay` is a JSON string (from the story editor) describing text
    // labels and freehand drawing strokes as normalized (0-1) coordinates,
    // so it renders correctly regardless of the viewer's screen size.
    let overlay = null;
    if (req.body.overlay) {
      try { overlay = JSON.parse(req.body.overlay); } catch (e) { overlay = null; }
    }
    ...
    const data = {
      ...
      music_url: req.body.music_url || null,
      music_name: req.body.music_name || null,
      music_artist: req.body.music_artist || null,
      music_start: req.body.music_start ? Number(req.body.music_start) || 0 : 0,
...
router.get('/feed', requireAuth, async (req, res, next) => {
  try {
    ...
    const groups = await Promise.all(orderedUserIds.map(async (userId) => {
      ...
      return {
        user: author,
        stories: stories.map(({ viewers, likes, ...rest }) => ({
          ...rest,
          viewedByViewer: (viewers || []).includes(req.user.id),
          likedByViewer: (likes || []).includes(req.user.id),
...
router.post('/:id/like', requireAuth, async (req, res, next) => {
  try {
    ...
    if (!likes.includes(req.user.id)) {
      await ref.update({ likes: [...likes, req.user.id] });
      if (data.user_id !== req.user.id) {
        await db.collection('notifications').add({
          user_id: data.user_id, actor_id: req.user.id, type: 'story_like',
          post_id: null, comment_id: null, story_id: req.params.id,
          is_read: false, created_at: new Date().toISOString(),
        });
      }
    }
    res.json({ liked: true, likeCount: likes.includes(req.user.id) ? likes.length : likes.length + 1 });
  } catch (err) { next(err); }
});
...
router.delete('/:id/like', requireAuth, async (req, res, next) => {
  try {
    ...
    if (!doc.exists) return res.status(404).json({ error: 'Story not found.' });
    const data = doc.data();
    const likes = (data.likes || []).filter((id) => id !== req.user.id);
    await ref.update({ likes });
    res.json({ liked: false, likeCount: likes.length });
  } catch (err) { next(err); }
});

// Who has liked one of your own stories (shown to the author only)
router.get('/:id/likes', requireAuth, async (req, res, next) => {
  try {
    const doc = await db.collection('stories').doc(req.params.id).get();
    if (!doc.exists) return res.status(404).json({ error: 'Story not found.' });
    const data = doc.data();
    if (data.user_id !== req.user.id) return res.status(403).json({ error: 'Only the author can see likes.' });
    const users = await Promise.all((data.likes || []).map((id) => authorFor(id)));
    res.json({ users });
  } catch (err) { next(err); }
});
