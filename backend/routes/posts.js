...
router.get('/explore', optionalAuth, async (req, res, next) => {
  try {
    ...
    const posts = await Promise.all(snap.docs.map((d) => attachPostExtras(d.id, d.data(), req.user && req.user.id)));
    res.json({ posts });
  } catch (err) { next(err); }
});
...
router.get('/user/:userId', optionalAuth, async (req, res, next) => {
  try {
    ...
    const posts = await Promise.all(docs.map((d) => attachPostExtras(d.id, d.data(), req.user && req.user.id)));
    res.json({ posts });
  } catch (err) { next(err); }
});
