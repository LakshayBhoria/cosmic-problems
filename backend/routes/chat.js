...
async function serializeConversation(conv, viewerId) {
  ...
  const base = {
    ...
    is_muted: (conv.muted_by || []).includes(viewerId),
...
// ---- mute -------------------------------------------------------------
// Per-viewer, like restrict — stops this conversation from generating
// notifications for you specifically; the other person is unaffected and
// unaware.

router.post('/conversations/:id/mute', requireAuth, async (req, res, next) => {
  try {
    const conv = await loadConversation(req.params.id);
    if (!conv) return res.status(404).json({ error: 'Conversation not found.' });
    assertParticipant(conv, req.user.id);
    await db.collection('conversations').doc(conv.id).update({
      muted_by: admin.firestore.FieldValue.arrayUnion(req.user.id),
    });
    res.json({ success: true, is_muted: true });
  } catch (err) { next(err); }
});

router.delete('/conversations/:id/mute', requireAuth, async (req, res, next) => {
  try {
    const conv = await loadConversation(req.params.id);
    if (!conv) return res.status(404).json({ error: 'Conversation not found.' });
    assertParticipant(conv, req.user.id);
    await db.collection('conversations').doc(conv.id).update({
      muted_by: admin.firestore.FieldValue.arrayRemove(req.user.id),
    });
