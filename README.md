...
**Stories** now have a full creation flow at `/create/story`: swipe between
a live Camera (tap to shoot, hold to record up to 15s) and Gallery, then an
editor to drop in draggable text and freehand drawing, and attach a track
from Jamendo's free music catalog (with a 15s trim picker) via a small
backend proxy at `GET /api/music/search` — set `JAMENDO_CLIENT_ID` in the
backend env to enable it (free key, see `.env.example`). Text/drawings are
stored as normalized coordinates (not baked into the image), so they render
correctly at any screen size in the story viewer, which now also plays the
attached track in sync with playback.
...
Stories, live video, ads, multi-factor auth, and delivered push
notifications (the notification *records* exist — wiring up actual push
delivery needs a service worker + provider) weren't implemented, to keep
this a finishable, deployable app rather than an open-ended project.

## Becoming an admin (for the verified-badge review panel)

Set `ADMIN_EMAILS` in the backend `.env` to a comma-separated list of
trusted emails before deploying — anyone logging in with one of those
addresses is auto-promoted to admin and gets an "Admin: verification
requests" link in Settings. From there, existing admins can promote or
revoke other admins via `POST`/`DELETE /api/admin/admins/:userId`.
