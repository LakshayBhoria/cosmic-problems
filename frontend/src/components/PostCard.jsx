...
export default function PostCard({ post, onChange, onDelete }) {
  ...
  const [activeModal, setActiveModal] = useState(null); // 'edit' | 'qr' | 'insights' | 'preview' | null
  ...
  const share = async () => {
    try {
      if (navigator.share) await navigator.share({ url: postUrl, title: caption || 'Cosmic Problems' });
