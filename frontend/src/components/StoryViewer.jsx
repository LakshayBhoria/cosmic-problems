...
import { X, ChevronLeft, ChevronRight, Trash2, Eye, Heart } from 'lucide-react';
...
export default function StoryViewer({ groups, startIndex, onClose, onExhausted }) {
  ...
  // Image stories with attached music get a longer window so the track has
  // room to breathe, capped at 15s.
  const imageDurationMs = story?.music_url ? 15000 : STORY_DURATION_MS;
  useEffect(() => {
    if (!story || paused || story.media_type === 'video') return;
    startRef.current = performance.now() - pausedAtRef.current;
    const tick = (now) => {
      const elapsed = now - startRef.current;
      const pct = Math.min(100, (elapsed / imageDurationMs) * 100);
      setProgress(pct);
      if (pct >= 100) {
  ...
  }, [story?.id, paused, goNextStory, imageDurationMs]);

  // Track the media box's rendered size so text/drawing overlays (stored as
  // 0-1 normalized coordinates) line up regardless of viewport size.
  useEffect(() => {
  ...
  // Redraw freehand strokes whenever the story or box size changes
  useEffect(() => {
    ...
    const paths = story?.overlay?.paths || [];
    paths.forEach((p) => {
  ...
  }, [story?.id, boxSize.w, boxSize.h, story?.overlay]);

  // Play/sync attached music with the active story
  useEffect(() => {
    const audio = audioRef.current;
    if (!audio) return;
    if (!story?.music_url) { audio.pause(); return undefined; }
    audio.src = mediaUrl(story.music_url);
    audio.currentTime = story.music_start || 0;
    audio.play().catch(() => {});
    return () => { audio.pause(); };
  }, [story?.id]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    const audio = audioRef.current;
    if (!audio || !story?.music_url) return;
    if (paused) audio.pause(); else audio.play().catch(() => {});
  }, [paused, story?.music_url]);
    ...
    <div className="fixed inset-0 z-50 bg-black flex items-center justify-center select-none">
      <div className="relative w-full h-full max-w-md mx-auto flex flex-col">
        ...
        >
          ...
          {/* Text + drawing overlay, stored as normalized (0-1) coordinates */}
          <canvas ref={canvasRef} className="absolute inset-0 pointer-events-none" />
          {(story.overlay?.texts || []).map((t) => (
            <div
              key={t.id}
              className="absolute pointer-events-none font-display font-semibold text-center whitespace-pre-wrap"
              style={{
                left: `${t.x * 100}%`, top: `${t.y * 100}%`,
                transform: `translate(-50%,-50%) rotate(${t.rotation || 0}deg)`,
                color: t.color, fontSize: `${(t.size || 0.075) * boxSize.w}px`,
                maxWidth: '85%', textShadow: '0 1px 6px rgba(0,0,0,0.5)',
              }}
            >
              {t.text}
            </div>
        ...
        {/* Bottom bar: viewer count (own stories only) + like button */}
        <div className="absolute bottom-4 left-2 right-2 z-20 flex items-end justify-between gap-2">
          ...
          {!isSelf && (
            <button
              ...
              className="flex items-center gap-1.5 text-white bg-black/40 rounded-full px-3 py-1.5 backdrop-blur active:scale-90 transition-transform"
            >
              <Heart size={20} className={liked ? 'fill-nebula-pink text-nebula-pink' : 'text-white'} strokeWidth={1.8} />
              {likeCount > 0 && <span className="text-xs font-medium">{likeCount}</span>}
            </button>
          )}
          {isSelf && likeCount > 0 && (
            <div className="flex items-center gap-1.5 text-white/90 text-xs bg-black/40 rounded-full px-3 py-1.5 backdrop-blur">
              <Heart size={13} className="fill-nebula-pink text-nebula-pink" /> {likeCount}
            </div>
          )}
        </div>
      </div>
