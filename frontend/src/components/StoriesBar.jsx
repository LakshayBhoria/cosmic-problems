...
export default function StoriesBar() {
  ...
  const onPick = async (e) => {
    ...
    try {
      ...
      alert(err.response?.data?.error || 'Could not upload your story.');
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <div className="max-w-lg mx-auto px-3 md:px-0 pt-4">
      <div className="flex gap-4 overflow-x-auto pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        <button
          onClick={() => (hasOwnStory ? setViewerIndex(myGroupIndex) : fileRef.current?.click())}
        ...
        >
          <div className={`relative w-16 h-16 rounded-full p-[2px] ${hasOwnStory && groups[myGroupIndex].hasUnseen ? 'bg-gradient-to-tr from-nebula-violet to-nebula-cyan' : hasOwnStory ? 'bg-nebula-border' : 'bg-transparent'}`}>
            ...
            >
              {uploading ? <span className="w-2.5 h-2.5 rounded-full border border-white border-t-transparent animate-spin" /> : <Plus size={12} strokeWidth={3} className="text-white" />}
            </span>
          </div>
