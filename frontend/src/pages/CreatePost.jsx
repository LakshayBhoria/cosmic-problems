...
export default function CreatePost() {
  ...
  const [type, setType] = useState(
    location.state?.type === 'reel' ? 'reel' : 'post'
  );
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (type === 'post') {
        fd.append('location', postLocation);
  ...
  return (
    <div className="max-w-lg mx-auto px-4 pt-6 pb-16">
      <h1 className="font-display text-xl font-semibold mb-4">
        Share a cosmic problem
      </h1>

      <div className="flex gap-2 mb-5">
        <button
          onClick={() => switchType('post')}
          className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl border text-sm ${
            type === 'post'
              ? 'border-nebula-violet text-nebula-violet bg-nebula-violet/10'
              : 'border-nebula-border text-nebula-muted'
          }`}
        >
      ...
      {error && (
        <p className="text-sm text-nebula-pink bg-nebula-pink/10 border border-nebula-pink/30 rounded-lg px-3 py-2 mb-3">
          {error}
        </p>
      )}
      ...
      {previews.length === 0 ? (
        <label className="card flex flex-col items-center justify-center gap-2 py-16 cursor-pointer border-dashed hover:border-nebula-violet/50 transition-colors">
          {type === 'post' ? (
            <ImagePlus size={32} className="text-nebula-muted" />
          ) : (
            <Clapperboard size={32} className="text-nebula-muted" />
          )}

          <p className="text-sm text-nebula-muted">
            {type === 'post'
              ? 'Select photos or videos (up to 10)'
              : 'Select a video'}
          </p>
      ...
      <div className="mt-3">
        ...
        >
          {(categories.length ? categories : ['General']).map(
            (c) => (
      ...
      {type === 'post' && (
        <div className="mt-3">
          ...
          <div className="flex items-center gap-2 input-field">
            <MapPin
              size={14}
              className="text-nebula-muted"
            />

            <input
              className="bg-transparent outline-none flex-1 text-sm"
              placeholder="Observatory, lab, or place"
              value={postLocation}
              onChange={(e) =>
