        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          {previews.map((p, i) => (
            <div key={i} className="relative aspect-square rounded-lg overflow-hidden bg-black">
              {p.type === 'video' ? <video src={p.url} className="w-full h-full object-cover" /> : <img src={p.url} className="w-full h-full object-cover" alt="" />}
      ...
      {type === 'post' && (
        <div className="mt-3">
          ...
          <div className="flex items-center gap-2 input-field">
            ...
            <input className="bg-transparent outline-none flex-1 text-sm" placeholder="Observatory, lab, or place" value={postLocation} onChange={(e) => setPostLocation(e.target.value)} />
          </div>
        </div>
      )}
