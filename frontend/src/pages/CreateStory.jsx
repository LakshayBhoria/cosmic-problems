        ...
        {screen === 'capture' && (
          <>
            <div className="absolute top-4 left-0 right-0 z-30 flex items-center justify-between px-4">
              <button onClick={() => navigate(-1)} className="p-1.5 rounded-full bg-black/40"><X size={22} /></button>
              <div className="flex items-center gap-5 text-xs font-display font-semibold tracking-wide">
                <button onClick={() => setMode('camera')} className={mode === 'camera' ? 'text-white' : 'text-white/50'}>CAMERA</button>
                <button onClick={() => setMode('gallery')} className={mode === 'gallery' ? 'text-white' : 'text-white/50'}>GALLERY</button>
              </div>
              {mode === 'camera' ? (
                <button onClick={() => setFacingMode((f) => (f === 'user' ? 'environment' : 'user'))} className="p-1.5 rounded-full bg-black/40">
                  <RefreshCw size={19} />
                </button>
              ) : <div className="w-8" />}
            </div>
            ...
            <div
              ...
              style={{ transform: mode === 'camera' ? 'translateX(0%)' : 'translateX(-50%)' }}
              onTouchStart={handleTouchStart}
              onTouchEnd={handleTouchEnd}
            >
              {/* Camera panel */}
              <div className="w-1/2 h-full relative bg-[#0b0c14] flex items-center justify-center">
                {cameraError ? (
                ...
                <p className="absolute bottom-1 left-0 right-0 text-center text-[10px] text-white/50">Tap for photo · hold for video</p>
              </div>

              {/* Gallery panel */}
              <div className="w-1/2 h-full relative bg-[#0b0c14] flex flex-col items-center justify-center gap-4 px-10">
                <button
                  onClick={() => galleryInputRef.current?.click()}
                  className="w-full aspect-square max-w-[220px] rounded-2xl border-2 border-dashed border-white/25 flex flex-col items-center justify-center gap-3 hover:border-white/50 transition-colors"
                >
                  <ImageIcon size={36} className="text-white/60" />
                  <span className="text-sm text-white/70">Choose from gallery</span>
                </button>
                <p className="text-xs text-white/40 text-center">Pick a photo or video from your device</p>
                <input ref={galleryInputRef} type="file" accept="image/*,video/*" hidden onChange={onGalleryPick} />
              </div>
            </div>
          </>
        )}
