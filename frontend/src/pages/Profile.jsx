...
import { BadgeCheck, Grid3x3, Clapperboard, Lock, Link as LinkIcon, PlayCircle, Pin, Archive } from 'lucide-react';
    ...
    <div className="max-w-3xl mx-auto px-3 md:px-6 pt-6">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {isSelf ? (
              <div className="flex gap-2">
                ...
                <Link to="/settings" className="btn-ghost px-3 py-1.5 text-sm hidden md:inline-block">Settings</Link>
              </div>
            ) : (
              <div className="flex gap-2">
                ...
                <button
                  onClick={async () => {
                    const { data } = await api.post('/chat/conversations', { user_id: profile.id });
      ...
      {!canSeeContent ? (
        ...
        <div className="grid grid-cols-3 gap-1 md:gap-2 mt-1 pb-8">
          ...
          {posts.map((p) => {
            const cover = p.media?.[p.cover_index] || p.media?.[0];
            return (
              <Link key={p.id} to={`/post/${p.id}`} className="relative aspect-square bg-nebula-surface overflow-hidden group">
                ...
                {p.is_pinned && <Pin size={14} className="absolute top-2 left-2 text-white drop-shadow" />}
                {p.is_archived && <Archive size={14} className="absolute bottom-2 left-2 text-white drop-shadow" />}
              </Link>
            );
          })}
        </div>
