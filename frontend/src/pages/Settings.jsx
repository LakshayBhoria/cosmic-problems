...
function VerificationPanel() {
  ...
  const loadStatus = useCallback(() => {
    ...
    refreshMe();
    api.get('/users/me/verification-request')
      .then(({ data }) => setRequest(data.request))
      .catch((e) => setLoadError(e.response?.data?.error || 'Could not load your verification status.'));
  }, []);
  ...
  if (loadError) {
    return (
      <div className="text-sm">
        <p className="text-nebula-pink mb-2">{loadError}</p>
        <button onClick={loadStatus} className="btn-ghost px-3 py-1.5 text-xs">Try again</button>
      </div>
    );
  }
