...
import { ArrowLeft, Send, MoreVertical, Pencil, Trash2, Users, UserPlus, LogOut, X, Music, BadgeCheck, Phone, Video } from 'lucide-react';
    ...
    <div className="max-w-md mx-auto flex flex-col h-[calc(100dvh-7.5rem)] md:h-[calc(100vh-6rem)]">
      <div className="sticky top-0 z-10 flex items-center gap-3 px-3 py-2.5 border-b border-nebula-border bg-nebula-bg shrink-0">
        ...
        {conv.is_group ? (
          ...
          <Link to={`/${title}`} className="w-9 h-9 rounded-full bg-nebula-surface border border-nebula-border overflow-hidden flex items-center justify-center shrink-0">
        ...
        {!conv.is_group && (
          <div className="flex items-center gap-3 shrink-0">
            <button onClick={() => showToast('Audio calls — coming soon')} className="text-nebula-text"><Phone size={19} /></button>
            <button onClick={() => showToast('Video calls — coming soon')} className="text-nebula-text"><Video size={20} /></button>
          </div>
        )}
