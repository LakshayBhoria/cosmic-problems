...
function NewChatModal({ onClose, onCreated, groupMode }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-void/94 backdrop-blur-sm flex flex-col items-center pt-16 px-4" onClick={onClose}>
      <div className="w-full max-w-md card p-0 overflow-hidden" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-4 border-b border-nebula-border">
          <h2 className="font-display font-semibold">{groupMode ? 'New group' : 'New message'}</h2>
...
export default function Chat() {
  ...
  // Deep link from elsewhere in the app (e.g. the contact-info panel's
  // "Create a group chat" entry) — /chat?new=group opens the modal directly.
  useEffect(() => {
    if (new URLSearchParams(loc.search).get('new') === 'group') setShowNewGroup(true);
  }, [loc.search]);
    ...
    <div className="max-w-md mx-auto px-3 md:px-0">
      <div className="flex items-center justify-between py-3">
        ...
        <div className="flex items-center gap-1">
          <button onClick={() => setShowNewGroup(true)} className="p-2 rounded-lg hover:bg-nebula-surface" aria-label="Create a group chat"><Users size={20} /></button>
          <button onClick={() => setShowNew(true)} className="p-2 rounded-lg hover:bg-nebula-surface" aria-label="New message"><SquarePen size={20} /></button>
        </div>
      </div>
      ...
      {showNewGroup && (
        <NewChatModal
          ...
          onCreated={(id) => { setShowNewGroup(false); nav(`/chat/${id}`); }}
        />
      )}
    </div>
  );
